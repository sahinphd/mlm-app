  ...
  <div class="flex flex-col justify-center flex-1 w-full max-w-md mx-auto">
    <div>
      <div class="mb-5 sm:mb-8">
        <h1 class="mb-2 font-semibold bg-white text-gray-800 text-title-sm dark:text-white/90 sm:text-title-md">
          Sign In
        </h1>
      ...
      <div>
        <form method="POST" action="{{ route('login') }}">
          @csrf
          @if(session('warning'))
            <div class="mb-4 p-3 rounded bg-warning-50 text-warning-600 border border-warning-100">{{ session('warning') }}</div>
          @endif
          <div class="space-y-5">
            <!-- Email -->
            <div>
