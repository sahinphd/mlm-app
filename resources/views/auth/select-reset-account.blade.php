  ...
  <div class="flex flex-col justify-center flex-1 w-full max-w-md mx-auto py-12 px-4 sm:px-0">
    <div class="mb-5 sm:mb-8">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">
          Multiple accounts found for this mobile number. Select the account you wish to reset.
        </p>
    </div>

    <form method="POST" action="{{ route('password.select_account.post') }}">
        @csrf
        <div class="space-y-4">
            ...
            <button type="submit" class="flex items-center justify-center w-full px-4 py-3 text-sm font-medium text-white transition rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600">
                Send Reset Link for Selected Account
            </button>
            
            <a href="{{ route('password.request') }}" class="block text-center text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400">
                Cancel
            </a>
        </div>
    </form>
