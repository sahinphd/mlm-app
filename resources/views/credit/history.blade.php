...
<div class="grid grid-cols-1 gap-4 md:gap-6">
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6 text-center">
        <h2 class="mb-3 text-2xl font-semibold text-gray-800 dark:text-white/90">Credit Account History</h2>
        <p class="mb-6 text-gray-500 dark:text-gray-400">View all your credit limit usage and repayment transactions.</p>
        
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-4xl mx-auto">
            <div class="rounded-2xl border border-gray-100 bg-brand-50/50 p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <h4 class="text-sm font-medium text-gray-500 mb-1">Credit Limit</h4>
                <p class="text-2xl font-bold text-brand-600 dark:text-white">₹{{ number_format($creditAccount->credit_limit ?? 0, 2) }}</p>
            </div>
            <div class="rounded-2xl border border-gray-100 bg-red-50/50 p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                <h4 class="text-sm font-medium text-gray-500 mb-1">Used Credit</h4>
                <p class="text-2xl font-bold text-red-600 dark:text-white">₹{{ number_format($creditAccount->used_credit ?? 0, 2) }}</p>
            </div>
            <div class="rounded-2xl border border-gray-100 bg-green-50/50 p-5 dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <p class="text-2xl font-bold text-green-600 dark:text-white">₹{{ number_format($creditAccount->available_credit ?? 0, 2) }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
        <h3 class="mb-5 text-xl font-semibold text-gray-800 dark:text-white/90">Credit Transaction Log</h3>
        
        <div class="overflow-x-auto">
            <table id="credit-table" class="w-full text-left">
                <thead>
...
<script>
    $(document).ready(function() {
        $('#credit-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('credit.history.data') }}",
            columns: [
