...
<div class="mx-auto max-w-270">
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <a href="{{ route('admin.commissions.export', request()->query()) }}" class="inline-flex items-center justify-center rounded-md bg-success py-2 px-6 text-center font-medium text-white hover:bg-opacity-90 lg:px-8 xl:px-10">
            Export to CSV
        </a>
    </div>
    <!-- Filter & Search -->
    <div class="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 mb-6">
        <form method="GET" action="{{ route('admin.commissions') }}" id="filterForm">
            <div class="flex flex-wrap items-center gap-4">
                ...
                <div class="w-40">
                    ...
                    <select name="type" class="w-full rounded border-[1.5px] border-stroke bg-transparent py-2 px-5 font-medium outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
                        <option value="">All Types</option>
                        <option value="joining" {{ $type === 'joining' ? 'selected' : '' }}>Joining</option>
                        <option value="repurchase" {{ $type === 'repurchase' ? 'selected' : '' }}>Repurchase</option>
                    </select>
                </div>
                ...
                <div class="flex items-end gap-2 pt-6">
                    ...
                    <a href="{{ route('admin.commissions') }}" class="flex justify-center rounded border border-stroke py-2 px-6 font-medium text-black hover:shadow-1 dark:border-strokedark dark:text-white transition">
                        Reset
                    </a>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Clear IDs if search text is manually cleared
            ...
            <table class="w-full table-auto">
                <thead>
                    <tr class="bg-gray-2 text-left dark:bg-meta-4">
                        ...
                        <th class="min-w-[120px] py-4 px-4 font-medium text-black dark:text-white text-right">Amount</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark text-right">
                            <p class="text-black dark:text-white font-bold">Rs.{{ number_format($comm->amount, 2) }}</p>
                        </td>
                        <td class="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                            @if($comm->type === 'joining')
                                <span class="inline-flex rounded-full bg-success bg-opacity-10 py-1 px-3 text-sm font-medium text-white">
                                    Joining
                                </span>
                            @else
                                <span class="inline-flex rounded-full bg-primary bg-opacity-10 py-1 px-3 text-sm font-medium text-white">
                                    Repurchase
                                </span>
                            @endif
                        </td>
